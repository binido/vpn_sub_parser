...
- «Проверить пинг» — параллельно (до 32 одновременно, таймаут 3 с) меряет время
  TCP-подключения к каждому серверу и раскрашивает чипы: до 150 мс зелёный,
  до 400 мс жёлтый, дальше красный, «нет ответа» у молчащих.
- Фильтр по имени и сортировка списка: как в подписке, по пингу или по имени.
...
- Проверка пинга меряет **TCP-подключение к серверу**, а не HTTP-запрос через
  прокси, как URL-тест в NekoBox: для второго внутри приложения должно жить
  ядро вроде xray-core. Задержку это показывает настоящую, но не доказывает,
  что ключ рабочий по авторизации. И учтите: если на машине уже поднят VPN или
  локальный прокси, отвечать на подключение может он — тогда все серверы
  покажут одинаково низкие числа.
